...
t = np.arange(0, 8, dt)
ff = 2.095676
sin = np.complex64(0.3 * np.exp(1j * 2 * np.pi * ff * t))
sin = np.repeat(np.expand_dims(sin, 0),  batch_size, axis=0)

with tf.device('/device:GPU:1'):
    gfnn = GFNN.GFNN(nosc, dt, use_hebbian_learning=True, avoid_nan=False)
...
ax1.imshow(np.real(sz[0,:,:]).T, cmap='gray')
ax1.set_yticks(xticks)
ax1.set_yticklabels(xlabels)

ax2.imshow(np.angle(sz[0,:,:]).T, cmap='gray')
