...
class GFNN():
    ...
    def noisy_zero_state(self, batch_size):
        ...
        if self._use_hebbian_learning:
            ...
            init_state = tf.concat([z_init, c_init], axis=-1)
    ...
    def _zdot(self, input_stim, state, ti):
        # Extract z and c states
        z = state
        x = tf.expand_dims(tf.gather(input_stim, ti, axis=-1), axis=-1)
        ...
        if self._use_hebbian_learning:
            ...
            x = tf.expand_dims(tf.gather(input_stim, ti, axis=-1), axis=-1) + tf.squeeze(tf.matmul(c_zero_diag, tf.expand_dims(z, axis=-1)), axis=-1) # Calculate input with Connections with x(t)
        ...
        if self._use_hebbian_learning:
            ...
            out_state = tf.concat([dzdt, tf.reshape(dcdt, [-1, self._num_osc * self._num_osc])], axis=-1)

        if self._avoid_nan:
            out_state = tf.where(tf.is_nan(tf.real(out_state)), tf.zeros_like(out_state), out_state)
            out_state = tf.where(tf.is_inf(tf.real(out_state)), self._c_limit * tf.ones_like(out_state), out_state)
