...
logname = 'with_ae-vgg-2fc-drop=0.5-' + dataset + ''.join(['-%s=%s' % (key, value) for (key, value) in train_params.items()])
...
lr_reducer = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.75, patience=5, verbose=1, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0)
...
model.summary()
loss = {'v1ae': tf.keras.losses.mean_absolute_error, 'v2ae': tf.keras.losses.mean_absolute_error, 'ecl_softmax': tf.keras.losses.categorical_crossentropy}
loss_weights = {'v1ae': 1.0, 'v2ae': 1.0, 'ecl_softmax': 1.0}
model.compile(loss=loss, loss_weights=loss_weights, optimizer=tf.keras.optimizers.Adam(lr=train_params['lr']), metrics={'ecl_softmax': utils.range_categorical_accuracy})
