...
def vggish(input, trainable=False, name=''):
    # Block 1
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(64, (3, 3), strides=(1, 1), activation='relu', padding='same', trainable=trainable), name=name+'conv1')(input)
    ...
    # Block 3
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(256, (3, 3), strides=(1, 1), activation='relu', padding='same', trainable=trainable), name=name+'conv3/conv3_1')(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(256, (3, 3), strides=(1, 1), activation='relu', padding='same', trainable=trainable), name=name+'conv3/conv3_2')(x)
    ...
    # Block 4
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(512, (3, 3), strides=(1, 1), activation='relu', padding='same', trainable=trainable), name=name+'conv4/conv4_1')(x)
