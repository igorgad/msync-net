...
v1_stft_t = tf.squeeze(modules.LogMel(params)(tf.squeeze(v1_input, axis=0)), axis=0)
v2_stft_t = tf.squeeze(modules.LogMel(params)(tf.squeeze(v2_input, axis=0)), axis=0)
...
for step in range(total_num_examples):
    ...
    pred_path = np.mean((path[0] - path[1]) * np.sqrt(3) // 2)
    label = label - acc_cost.shape[0]//2
    preds.append(pred_label)
    preds_path.append(pred_path)
    labels.append(label)
    
    print ('step %d: [%d %d] - %d' % (step, pred_label, pred_path, label))
