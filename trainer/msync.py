...
[parser.add_argument('--%s' % key, type=type(val[0]), help='%s' % val, default=val, nargs='+') for key, val in train_params.items() if type(val) == list]
[parser.add_argument('--%s' % key, type=type(val[0]), help='%s' % val, default=val, nargs='+') for key, val in model_params.items() if type(val) == list]
[parser.add_argument('--%s' % key, type=type(val[0]), help='%s' % val, default=val, nargs='+') for key, val in data_params.items() if type(val) == list]
[parser.add_argument('--%s' % key, type=type(val), help='%s' % val, default=val) for key, val in train_params.items() if not type(val) == list]
[parser.add_argument('--%s' % key, type=type(val), help='%s' % val, default=val) for key, val in model_params.items() if not type(val) == list]
[parser.add_argument('--%s' % key, type=type(val), help='%s' % val, default=val) for key, val in data_params.items() if not type(val) == list]
