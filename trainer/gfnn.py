...
class GFNNLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        z_state = tf.squeeze(z_state, axis=-1)

        z_state.set_shape([inputs.shape[0], inputs.shape[1], self.gfnn._num_osc])
        z_state = tf.pow(z_state, 2)
    ...
    def compute_output_shape(self, input_shape):
        return tf.TensorShape((input_shape[0],  input_shape[1], self.gfnn._num_osc))
