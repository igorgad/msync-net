...
logname = '2master-lstm_featmap/kfold/' + ''.join(['%s=%s/' % (key, str(val).replace('/', '').replace(' ', '').replace('gs:', '')) for key, val in sorted(list(params.__dict__.items()))]) + 'run'
...
for k in range(params.num_folds):
    ...
    with tf.device('/device:CPU:0'):
        ...
        cmats = [tf.Variable(tf.zeros([params.example_length // params.stft_step + 1, params.example_length // params.stft_step + 1], dtype=tf.int32), trainable=False) for _ in [1, 5]]
    ...
    if logaudio:
        ...
        sum_list.append(synced_sum)
        
    merged_sum = tf.summary.merge(sum_list)
