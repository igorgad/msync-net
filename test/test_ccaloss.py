...
def cca_loss(outdim_size, use_all_singular_values):
    def inner_cca_loss(y_true, y_pred):
        batch_corr = tf.map_fn(lambda bi: compute_batch_loss(y_pred[bi], outdim_size, use_all_singular_values), tf.range(tf.shape(y_pred)[0]),
                               dtype=tf.float32, parallel_iterations=128)
        return tf.reduce_mean(batch_corr)

    return inner_cca_loss


y_pred = tf.convert_to_tensor(np.float32(np.random.randn(1024, 1024, 256)))

sess = tf.Session()

st = time.time()
r = sess.run(cca_loss(256, True)(y_pred, y_pred))
print(time.time() - st)
