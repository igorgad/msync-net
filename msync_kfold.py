...
# K fold validation
for k in range(data_params['num_folds']):
    test_folds = k
    val_folds = (k + 1) % data_params['num_folds']
    train_folds = np.setdiff1d(np.arange(data_params['num_folds'], dtype=np.int32), np.array([val_folds, test_folds]))
    train_data, validation_data, test_data = dts.kfold_pipeline(data_params, train_folds=train_folds, val_folds=val_folds, test_folds=test_folds)

    checkpoint = tf.keras.callbacks.ModelCheckpoint('./logs/%s/knum%d/model-checkpoint.hdf5' % (logname, k), monitor='val_loss', period=1, save_best_only=True)
    early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=10, verbose=1, mode='auto')
    tensorboard = stats.TensorBoardAVE(log_dir='./logs/%s/knum%d' % (logname, k), histogram_freq=8, batch_size=data_params['random_batch_size'], write_images=True)
    lr_reducer = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10, verbose=1, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0)
    ...
    hist = model.fit(train_data, epochs=2, steps_per_epoch=25, validation_data=validation_data, validation_steps=25)  #, callbacks=callbacks)
    test_res = model.evaluate(test_data, steps=25)

    fit_hist.append(hist)
    test_hist.append(test_res)
...
print('FINAL TEST ACCURACY AVERAGE: ' + str(test_hist.mean(0)[2]))